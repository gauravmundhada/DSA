...
/*
observations : minimize the maximum pattern (bs on ans)
we will bs on edges
range will be the least and max edge
for every mid we will check if the path has >= mid edges cost
since "score as the minimum edge‑cost along that path."
we will also need to keep track of total cost to check it don't exceeds k and offline nodes too
